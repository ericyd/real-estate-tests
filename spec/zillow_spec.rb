...
# Define a convenience class for handling the deeply nested response body
class ResponseBody
  attr_reader :address
...
RSpec.describe "Zillow API" do
  ...
  it "should search by zipcode" do
    ...
    # get API
    res = @conn.get '/webservice/GetDeepSearchResults.htm', @zwsid.merge({
      'address' => address,
    ...
    expect(body.get_message_text).to eq('Request successfully processed')
    expect(body.get_message_code).to eq('0')

    # verify result set
    expect(body.get_street).to eq(address)
    expect(body.get_zipcode).to eq(citystatezip)
  end
  ...
  it "should search by city/state" do
    ...
    # verify result set
    expect(body.get_street).to eq(address)
    expect(body.get_city).to eq(citystatezip.split(",")[0])
    expect(body.get_state).to eq(citystatezip.split(",")[1])
  end
  ...
  it "should notify client when no matches found" do
    ...
    body = ResponseBody.new(res.body)
    expect(res.status).to eq(200)
    expect(body.get_message_text).to eq('Error: no exact match found for input address')
    expect(body.get_message_code).to eq('508')
  ...
  it "should notify client when invalid ZWS-ID is used" do
    ...
    body = ResponseBody.new(res.body)
    expect(res.status).to eq(200)
    expect(body.get_message_text).to eq('Error: invalid or missing ZWSID parameter')
    expect(body.get_message_code).to eq('2')
    expect(body.address.nil?).to eq(true)
  end
  ...
  it "gets property details by ZPID" do
    ...
    # get API
    res = @conn.get '/webservice/GetUpdatedPropertyDetails.htm', @zwsid.merge({
      'zpid' => zpid
    })
